...
public class IPSeeker {
    ...
    public IPSeeker(Logger logger,
            RegistryShutdownHub shutdownHub,
            List<Resource> contributions) {
    	this.logger = logger;
        ...
        b4 = new byte[4];
        b3 = new byte[3];
        for (Resource resource: contributions)
    	{
	        try {
	        	ipFile = new RandomAccessFile(new File(resource.toURL().toURI()), "r");
	        } catch (FileNotFoundException e) {
	        	logger.error("IP地址信息文件没有找到，IP显示功能将无法使用" + resource.getFile(), e);
	            ipFile = null;
    ...
    public String getCountry(byte[] ip) {
        ...
        if(ipCache.containsKey(ipStr)) {
            ...
            return filterCounty(loc.country);
        } else {
    ...
    /**
     * 过滤县名
     * 如：福建省福州市永泰县	过滤成——>	福建省福州市
     */
    private String filterCounty(String location){
    	if (location.contains("市")) {
    		return location.substring(0,location.indexOf("市")+1);
		}
    	if (location.contains("地区")){//过滤新疆地名
			return location.substring(0,location.indexOf("区")+1);
		}
    	return location;
    }
