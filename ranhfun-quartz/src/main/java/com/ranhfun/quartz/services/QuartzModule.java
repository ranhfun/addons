...
import com.ranhfun.quartz.RAMSchedulerConstants;

public class QuartzModule {
	
    public static void bind(ServiceBinder binder)
    {
    	binder.bind(JobFactory.class, QuartzJobFactory.class);
    }
	...
	public static void provideFactoryDefaults(
		...
		configuration.add(RAMSchedulerConstants.QUARTZ_RAM_FILE, "quartz-ram.properties");
	}
    
    @Marker(RAMScheduler.class)
    public SchedulerFactory buildRAMSchedulerFactory(Logger logger,
            RegistryShutdownHub shutdownHub, List<JobSchedulingBundle> jobSchedulingBundles,
            @Symbol(RAMSchedulerConstants.QUARTZ_RAM_FILE)
            String quartzFile) { 
        if (logger.isInfoEnabled())
            logger.info("initialize RAM Scheduler factory");
    ...
    @Marker(RAMScheduler.class)
    @EagerLoad
	public static Scheduler buildRAMScheduler(
			@RAMScheduler
