...
public class ProxoolConnectionProvider implements ConnectionProvider, Configurable, Stoppable {
	...
	public void configure(Properties props) throws HibernateException {
		...
		if (ProxoolUtil.decryptFlag()) {
			try {
				ConnectionPoolDefinitionIF cp = ProxoolFacade.getConnectionPoolDefinition(proxoolAlias.substring(PROXOOL_JDBC_STEM.length()));
				Properties info = new Properties();
				info.setProperty(ProxoolConstants.USER_PROPERTY, ProxoolUtil.decrypt(cp.getUser()));
				info.setProperty(ProxoolConstants.PASSWORD_PROPERTY, ProxoolUtil.decrypt(cp.getPassword()));
				ProxoolFacade.updateConnectionPool(cp.getCompleteUrl(), info);
			} catch (ProxoolException e) {
				String msg = "ConnectionPoolDefinitionIF custom fail";
	            LOG.error(msg, e);
				throw new HibernateException( msg, e );
			}
		}
	...
	public void configure(Map configurationValues) {
		...
		for (Object entry : configurationValues.entrySet()) {
