...
KE.setting = {
	...
	items : [
		...
		'italic', 'underline', 'strikethrough', 'removeformat', '|', 'image', 'file',
...
KE.plugin['file'] = {
		getSelectedNode : function(id) {
			...
			if (!KE.browser.WEBKIT && !g.keSel.isControl) return;
			if (startNode.nodeType != 1) return;
			if (startNode.tagName.toLowerCase() != 'file') return;
			if (startNode != endNode) return;
			if (!startNode.className.match(/^ke-\w+/i)) return startNode;
		},
		init : function(id) {
		...
		check : function(id) {
			var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
			var type = KE.$('type', dialogDoc).value;
			var title = KE.$('fileTitle', dialogDoc).value;
			var urlBox;
			if (type == 2) {
				urlBox = KE.$('fileFile', dialogDoc);
			} else {
				urlBox = KE.$('url', dialogDoc);
			}
			if (!urlBox.value.match(/\.(zip|rar|txt|xml|xls|doc|pdf)(\s|\?|$)/i)) {
