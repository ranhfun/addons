...
public class AsyncEventBus extends DefaultEventBus {
  ...
  public AsyncEventBus(String identifier, Executor executor) {
    ...
    logger = Logger.getLogger(AsyncEventBus.class.getName() + "." + checkNotNull(identifier));
  }
  ...
  public AsyncEventBus(Executor executor) {
    this.executor = checkNotNull(executor);
    logger = Logger.getLogger(AsyncEventBus.class.getName());
  }
            ...
            try {
				...
				// should never occur
			      logger.log(Level.SEVERE,
			              "Could not dispatch async event: " + event + " to handler " + handler, e);
			}
          }
        });
  }
