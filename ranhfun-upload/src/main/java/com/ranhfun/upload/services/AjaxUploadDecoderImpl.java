...
import com.ranhfun.upload.UploadComponentConstants;
...
public class AjaxUploadDecoderImpl implements AjaxUploadDecoder {
    ...
    public boolean isAjaxUploadRequest(HttpServletRequest request) {
        ...
        return request.getParameter(AJAX_UPLOAD_HEADER) != null;
    }

    public boolean isAjaxUploadRequest(Request request) {

        return request.isXHR() && request.getParameter(AJAX_UPLOAD_HEADER) != null;
    }

    public void setupUploadedFile(HttpServletRequest request) {

        String fieldName = request.getParameter(AJAX_UPLOAD_HEADER);
        FileItem item = fileItemFactory.createItem(fieldName, request.getContentType(), false, request.getParameter(UploadComponentConstants.IMAGE_UPLOAD_PARAMETER));
        try {
